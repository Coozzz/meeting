...
public class MenuView extends FrameLayout {
	...
	public MenuView.BarrageViewListener mBarrageViewListener = new MenuView.BarrageViewListener() {
		@Override
		public void onMessageArrive(IMMessageBean object){
	...
	public MultiSpeakerViewListener mMultiSpeakerViewListener = new MultiSpeakerViewListener(){
		@Override
		public void add(int position){
			mMainButtonView.addDotTextView(position);
		}
		@Override
		public void remove(int position){
			mMainButtonView.removeDocTextView(position);
		}
		@Override
		public void update(int position){
			mMainButtonView.updateDotTextView(position);
		}
	...
	public void init(MenuViewListener menuViewListener,
		...
		mMultiSpeakerView = new MultiSpeakerView(mButelOpenSDK, mContext, this,
				accountId, density,mMultiSpeakerViewListener) {
				...
				if (v.getId() == MResource.getIdByName(mContext, MResource.ID,
					...
					if (mIsSpeak) {
						if (mButelOpenSDK.getMeetingMode() == MeetingStyleStatus.MASTER_MODE) {
							CustomToast.show(mContext, getContext().getString(R.string.host_order_speak),
...
//				}
	...
	public void showGiveMicView(List<Person> participatorList) {
		...
		if (mGiveMicList.size() == 0) {
			show(mContext, getContext().getString(R.string.no_give_mic_people), CustomToast.LENGTH_SHORT);
			return;
		}
